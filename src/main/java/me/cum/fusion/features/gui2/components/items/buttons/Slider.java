...
public class Slider extends Button
{
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        RenderUtil.drawRect(this.x, this.y, (this.setting.getValue().floatValue() <= this.min.floatValue()) ? this.x : (this.x + (this.width + 7.4f) * this.partialMultiplier()), this.y + this.height - 0.5f, this.isHovering(mouseX, mouseY) ? Fusion.colorManager.getColorWithAlpha(Fusion.moduleManager.getModuleByClass(NewGUI.class).alpha.getValue()) : Fusion.colorManager.getColorWithAlpha(Fusion.moduleManager.getModuleByClass(NewGUI.class).hoverAlpha.getValue()));
        Fusion.textManager.drawStringWithShadow(this.getName() + " " + ChatFormatting.GRAY + ((this.setting.getValue() instanceof Float) ? this.setting.getValue() : this.setting.getValue().doubleValue()), this.x + 2.3f, this.y - 1.7f - ClickGuiScreen.getClickGui().getTextOffset(), -1);
    }
