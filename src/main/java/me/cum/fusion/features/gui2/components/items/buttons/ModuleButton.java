...
public class ModuleButton extends Button
{
    ...
    public ModuleButton(final Module module) {
        ...
        this.logo = new ResourceLocation("fusion.png");
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        if (!this.items.isEmpty()) {
            if (HUD.getInstance().fusionImage.getValue()) {
                ModuleButton.mc.getTextureManager().bindTexture(this.logo);
                drawCompleteImage(this.x - 1.5f + this.width - 7.4f, this.y - 2.2f - ClickGuiScreen.getClickGui().getTextOffset(), 8, 8);
            }
            if (this.subOpen) {
                float height = 1.0f;
