...
package me.cum.fusion.features.gui2.components;
...
public class Component extends Feature
{
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        final int color = ColorUtil.toARGB(NewGUI.getInstance().topRed.getValue(), NewGUI.getInstance().topGreen.getValue(), NewGUI.getInstance().topBlue.getValue(), 255);
        Gui.drawRect(this.x, this.y - 1, this.x + this.width, this.y + this.height - 6, ((boolean)NewGUI.getInstance().rainbow.getValue()) ? ColorUtil.rainbow(NewGUI.getInstance().rainbowHue.getValue()).getRGB() : color);
        if (this.open) {
            RenderUtil.drawRect((float)this.x, this.y + 12.5f, (float)(this.x + this.width), this.y + this.height + totalItemHeight, 1996488704);
        }
        Fusion.textManager.drawStringWithShadow(this.getName(), this.x + 3.0f, this.y - 4.0f - ClickGuiScreen.getClickGui().getTextOffset(), -1);
        if (this.open) {
