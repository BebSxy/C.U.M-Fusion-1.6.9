...
public class AutoSex extends Module
{
    ...
    public AutoSex() {
        super("AutoDildo", "fucking horny", Module.Category.TROLL, false, false, false);
        this.dildoProgess = 0.0f;
    }
    
    public void onRender3D(final Render3DEvent render3DEvent) {
        this.dildoProgess += (float)0.1;
        for (final Object e : AutoSex.mc.world.loadedEntityList) {
            if (!(e instanceof EntityPlayer)) {
                continue;
            }
            final RenderManager renderManager = Minecraft.getMinecraft().getRenderManager();
            final EntityPlayer entityPlayer = (EntityPlayer)e;
            final double d = entityPlayer.lastTickPosX + (entityPlayer.posX - entityPlayer.lastTickPosX) * AutoSex.mc.timer.renderPartialTicks;
            final double d2 = d - renderManager.renderPosX;
            final double d3 = entityPlayer.lastTickPosY + (entityPlayer.posY - entityPlayer.lastTickPosY) * AutoSex.mc.timer.renderPartialTicks;
            final double d4 = d3 - renderManager.renderPosY;
            final double d5 = entityPlayer.lastTickPosZ + (entityPlayer.posZ - entityPlayer.lastTickPosZ) * AutoSex.mc.timer.renderPartialTicks;
    ...
    public void esp(final EntityPlayer entityPlayer, final double d, final double d2, final double d3) {
        ...
        GL11.glTranslated(0.0, 0.0, (double)(-4.075f + this.dildoProgess));
        final Cylinder cylinder = new Cylinder();
        cylinder.setDrawStyle(100013);
        cylinder.draw(0.1f, 0.11f, 1.0f, 25, 20);
        GL11.glColor4f(ColorUtil.rainbow(50).getRed() / 255.0f, ColorUtil.rainbow(50).getGreen() / 255.0f, ColorUtil.rainbow(50).getBlue() / 255.0f, 1.0f);
        GL11.glTranslated(0.0, 0.0, -0.12500000298023223 + this.dildoProgess / 14.0f);
