...
public class CrystalChamsRewrite extends Module
{
    ...
    public CrystalChamsRewrite() {
        super("CrystalModifier", "Modifies crystal rendering in different ways", Module.Category.RENDER, true, true, false);
        ...
        CrystalChamsRewrite.INSTANCE = this;
    }
    
    public void onUpdate() {
        for (final Entity crystal : CrystalChamsRewrite.mc.world.loadedEntityList) {
    ...
    public void onReceivePacket(final PacketEvent.Receive event) {
        if (event.getPacket() instanceof SPacketDestroyEntities) {
            ...
            for (final int id : packet.getEntityIDs()) {
                final Entity entity = CrystalChamsRewrite.mc.world.getEntityByID(id);
    ...
    public void onRenderModel(final RenderEntityModelEvent event) {
        ...
        final Color color = this.colorSync.getValue() ? ClickGui.getInstance().getCurrentColor() : EntityUtil.getColor(event.entity, this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.alpha.getValue(), false);
        final boolean fancyGraphics = CrystalChamsRewrite.mc.gameSettings.fancyGraphics;
        CrystalChamsRewrite.mc.gameSettings.fancyGraphics = false;
        final float gamma = CrystalChamsRewrite.mc.gameSettings.gammaSetting;
        CrystalChamsRewrite.mc.gameSettings.gammaSetting = 10000.0f;
        GL11.glPushMatrix();
