...
public class ConfigManager implements Util
{
    ...
    public static void setValueFromJson(final Feature feature, final Setting setting, final JsonElement element) {
        ...
        switch (type) {
            ...
            default: {
                Fusion.LOGGER.error("Unknown Setting type for: " + feature.getName() + " : " + setting.getName());
            }
        }
    }
    ...
    private static void loadFile(final JsonObject input, final Feature feature) {
        for (final Map.Entry<String, JsonElement> entry : input.entrySet()) {
            ...
            if (feature instanceof FriendManager) {
                try {
                    Fusion.friendManager.addFriend(new FriendManager.Friend(element.getAsString(), UUID.fromString(settingName)));
                }
                catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public void loadConfig(final String name) {
        final List<File> files = Arrays.stream((Object[])Objects.requireNonNull((T[])new File("fusion").listFiles())).filter(File::isDirectory).collect((Collector<? super Object, ?, List<File>>)Collectors.toList());
        if (files.contains(new File("fusion/" + name + "/"))) {
            this.config = "fusion/" + name + "/";
        }
        else {
            this.config = "fusion/config/";
        }
    ...
    public boolean configExists(final String name) {
        final List<File> files = Arrays.stream((Object[])Objects.requireNonNull((T[])new File("fusion").listFiles())).filter(File::isDirectory).collect((Collector<? super Object, ?, List<File>>)Collectors.toList());
        return files.contains(new File("fusion/" + name + "/"));
    }
    ...
    public void saveConfig(final String name) {
        ...
        Fusion.friendManager.saveFriends();
        for (final Feature feature : this.features) {
            try {
    ...
    public void init() {
        this.features.addAll((Collection<? extends Feature>)Fusion.moduleManager.modules);
        this.features.add(Fusion.friendManager);
        final String name = this.loadCurrentConfig();
        this.loadConfig(name);
        Fusion.LOGGER.info("Config loaded.");
    }
    ...
    private void loadPath(final Path path, final Feature feature) throws IOException {
        ...
        try {
            ...
            Fusion.LOGGER.error("Bad Config File for: " + feature.getName() + ". Resetting...");
        ...
        for (final Setting setting : feature.getSettings()) {
            if (setting.isEnumSetting()) {
                ...
                if (setting.isStringSetting()) {
                    ...
                    setting.setValue((Object)str.replace(" ", " "));
                }
                try {
