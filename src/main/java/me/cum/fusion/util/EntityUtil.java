...
public class EntityUtil implements Util
{
    ...
    public static double getBaseMoveSpeed() {
        ...
        if (EntityUtil.mc.player != null && EntityUtil.mc.player.isPotionActive((Potion)Objects.requireNonNull(Potion.getPotionById(1)))) {
            final int amplifier = Objects.requireNonNull(EntityUtil.mc.player.getActivePotionEffect((Potion)Objects.requireNonNull(Potion.getPotionById(1)))).getAmplifier();
    ...
    public static List<EntityPlayer> getNearbyPlayers(final double d) {
        ...
        list.removeIf(entityPlayer -> Fusion.friendManager.isFriend(entityPlayer.getName()));
        return list;
    }
    ...
    public static boolean isntValid(final Entity entity, final double range) {
        return entity == null || isDead(entity) || entity.equals((Object)EntityUtil.mc.player) || (entity instanceof EntityPlayer && Fusion.friendManager.isFriend(entity.getName())) || EntityUtil.mc.player.getDistanceSq(entity) > MathUtil.square(range);
    }
    ...
    public static Color getColor(final Entity entity, final int red, final int green, final int blue, final int alpha, final boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer && colorFriends && Fusion.friendManager.isFriend((EntityPlayer)entity)) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            if (!player.isInvisible()) {
                ...
                output.put((Object)healthSB + " " + (Fusion.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + (Object)distanceSB + " \u00c2§f0", (int)EntityUtil.mc.player.getDistance((Entity)player));
