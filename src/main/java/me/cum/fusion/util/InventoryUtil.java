...
public class InventoryUtil implements Util
{
    ...
    public static void switchToHotbarSlot(final Class<? extends IForgeRegistryEntry.Impl> clazz, final boolean silent) {
    ...
    public static int findHotbarBlock(final Class<? extends IForgeRegistryEntry.Impl> clazz) {
        for (int i = 0; i < 9; ++i) {
    ...
    public static int findInventoryBlock(final Class<? extends Block> clazz, final boolean offHand) {
        final AtomicInteger slot = new AtomicInteger();
        slot.set(-1);
        for (final Map.Entry<Integer, ItemStack> entry : getInventoryAndHotbarSlots().entrySet()) {
    ...
    public static boolean isBlock(final Item item, final Class<? extends Block> clazz) {
    ...
    public static boolean[] switchItem(final boolean back, final int lastHotbarSlot, final boolean switchedItem, final Switch mode, final Class<? extends IForgeRegistryEntry.Impl> clazz) {
        ...
        switch (mode) {
            ...
            case SILENT: {
                if (!back && !switchedItem) {
                    ...
                    switchedItemSwitched[0] = false;
                    Fusion.inventoryManager.recoverSilent(lastHotbarSlot);
                }
                switchedItemSwitched[1] = true;
                break;
            }
    ...
    public static int findBlockSlotInventory(final Class<Block> clazz, final boolean offHand, final boolean withXCarry) {
